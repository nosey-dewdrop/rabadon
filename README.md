...
**Fourteen times a week it stops something that cannot be undone.** That is the number worth quoting, and it is not the big one: of 521 refusals in 29 days, 439 were recoverable — a `wip` commit message, an action on a red base — the kind of rule anyone writes in a shell function in an afternoon, and nobody installs a tool for. The other 82 were irreversible: a force-push, a delete reaching outside the tree, `git gc --prune=now`, a `sed -i` over source with no backup, a rule moved into `disabled[]`. 57 of those 82 were on real work rather than this project's own sandboxes — 13.8 a week. Reproduce: `python3 bench/irreversible.py`. Honest reading, including who will see far fewer: [BENCHMARK.md §3d](BENCHMARK.md).
