...
macOS + Linux, Node ≥ 18. The core is ~23k lines of dependency-free C++; prebuilt binaries ship per platform, and if none matches, the postinstall builds from source with `clang++`/`g++` (`rabadon doctor` diagnoses either way). Full walkthrough: [docs/quickstart.md](docs/quickstart.md).
...
**Ten times a week it stops something that cannot be undone.** That is the number worth quoting, and it is not the big one: of 521 refusals in 29 days, 439 were recoverable — a `wip` commit message, an action on a red base — the kind of rule anyone writes in a shell function in an afternoon, and nobody installs a tool for. Another 16 deferred a push until the suite was green, which loses nothing. The remaining 66 were irreversible: a force-push, a delete reaching outside the tree, `git gc --prune=now`, a `sed -i` over source with no backup, a rule moved into `disabled[]`. 41 of those were on real work rather than this project's own sandboxes — 9.9 a week. Reproduce: `python3 bench/irreversible.py`. Honest reading, including who will see far fewer: [BENCHMARK.md §3d](BENCHMARK.md).

**It refuses about one honest action in 126.** A guard that gets in the way is uninstalled by lunch, so the number is published either way: 126 refusals since 28 August, 1 of them called wrong by the operator with `rabadon wrong` — 0.8 %. Over all 29 days with traffic it is 521 and 70, or 13.4 %, and 69 of those 70 fell on two days in August while three rules were being tuned against real work. Reproduce on your own ledger, offline: `python3 bench/precision.py --since 2026-08-28` for the 0.8 % row, or the bare command for the whole history. What it is not: one operator, one machine — a stranger's rate on a codebase rabadon has never seen is unmeasured. Detail: [BENCHMARK.md §3b](BENCHMARK.md).
